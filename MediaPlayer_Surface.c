...
Link with gcc -Wall -o "%e" "%f" -D_POSIX_C_SOURCE=199309L $(pkg-config --cflags gtk+-3.0) -Wl,--whole-archive -I/opt/vc/include -L/opt/vc/lib/ -lGLESv2 -lEGL -lbcm_host -lvchiq_arm -lpthread -lrt -ldl -lm -Wl,--no-whole-archive -rdynamic $(pkg-config --libs gtk+-3.0) $(pkg-config --libs libavcodec libavformat libavutil libswscale) -lasound
*/
...
void init_ogl(CUBE_STATE_T *state)
{
    ...
    // SD PAL 4:3 768*576, SD PAL 16:9 768*432, HD 16:9 1280*720, Full HD 16:9 1920*1080

    p_state->screen_width = 768; // 768 //1280;
    p_state->screen_height = 576; // 432 //720;
...
//printf("free vq\n");
	...
	printf("Played %ld, skipped %ld, frame drop rate %3.2f\n", played, skipped, (float)skipped/(float)played);
...
{
	setup_default_icon("./MediaPlayer_Surface.png");
